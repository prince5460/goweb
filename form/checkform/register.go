...
func register(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "GET" {
		f, _ := template.ParseFiles("register.html")
		f.Execute(w, nil)
	} else {
		...
		//3.验证中文
		if matched, _ := regexp.MatchString(`^[\x{4e00}-\x{9fa5}]+$`, r.Form.Get("zhname")); !matched {
			fmt.Println("验证有误，请输入中文！")
			fmt.Fprintf(w, "验证有误，请输入中文！")
		}

		//4. 验证英文
		if matched, _ := regexp.MatchString("^[a-zA-Z]+$", r.Form.Get("enname")); !matched {
			fmt.Println("验证有误，请输入英文！")
			fmt.Fprintf(w, "验证有误，请输入英文！")
		}

		//5. 邮箱
		if matched, _ := regexp.MatchString(`^([\w\.\_]{2,10})@(\w{1,}).([a-z]{2,4})$`, r.Form.Get("email")); !matched {
			fmt.Println("请输入正确邮箱地址")
			fmt.Fprintf(w, "验证有误，请输入正确邮箱地址！")
		}

		//6. 验证手机号
		if matched, _ := regexp.MatchString(`^(1[3|5|6|7|8][0-9]\d{8})$`, r.Form.Get("mobile")); !matched {
